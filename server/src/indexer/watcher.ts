...
export function startWorkspaceWatcher(): void {
  if (!config.INDEXER_ENABLED || !config.INDEXER_WATCHER_ENABLED) {
    log.info('Workspace watcher skipped', {
      indexerEnabled: config.INDEXER_ENABLED,
      watcherEnabled: config.INDEXER_WATCHER_ENABLED,
    })
    return
  }
  if (watcher) return
