...
export function formatCoworkHeadlessUnsupportedMessage(exe: string): string {
  ...
  return (
    ...
    `(Open Cowork main branch / a newer release). ` +
    `Build from source (e.g. BMC/center/open-cowork/raw/open-cowork-main) and set OPEN_COWORK_EXE ` +
    `to that Electron binary, or watch ${OPEN_COWORK_RELEASES_URL}. ` +
    `Chat and panel will keep writing inbox briefs, but headless runs cannot start until then.`
  )
}
  ...
  private async runTaskInner(
    ...
    const done = new Promise<void>((resolve, reject) => {
      ...
      const onChildExit = (code: number | null, signal: NodeJS.Signals | null) => {
        // Soft-abort / stop during a run should settle the promise, not hang until timeout.
        const err = new Error(
          `cowork stopped during task ${taskId} (code ${code}${signal ? ` signal ${signal}` : ''}): ${this.stderrTail()}`,
        )
        this.crashed = err
        this.started = false
        finish(() => reject(err))
