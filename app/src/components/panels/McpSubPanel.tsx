...
/** Curated Best options — picking one prefills Add MCP server (persists via /api/settings/mcp). */
function BestMcpOptions({ onPick }: { onPick: (preset: McpPreset) => void }) {
  return (
      ...
      <div className="flex items-baseline justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-fg-muted">
            Start from a recommended transport. Create writes to Postgres (`mcp_servers`); connect
            events land in `mcp_connection_events`.
          </p>
        </div>
      </div>
        ...
        {BEST_MCP_PRESETS.map((preset) => {
          ...
          return (
            <button
              ...
              className="rounded-lg border border-border p-3 text-left transition-colors hover:border-accent hover:bg-accent-muted/40"
            >
...
export function McpSubPanel() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery({
    ...
    queryFn: () => apiClient.get<McpListResponse>('/api/settings/mcp'),
    retry: false,
    refetchInterval: (q) => {
  ...
  const toggle = useMutation({
    mutationFn: (server: McpServer) =>
      apiClient.patch<McpServer>(`/api/settings/mcp/${server.id}`, { enabled: !server.enabled }),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['mcp-servers'] }),
    onError: () => addToast({ title: 'Update failed', variant: 'danger' }),
  })

  const remove = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/api/settings/mcp/${id}`),
    onSuccess: () => {
  ...
  const test = useMutation({
    mutationFn: (id: string) =>
      apiClient.post<McpTestResponse>(`/api/settings/mcp/${id}/test`),
    onSuccess: (res) => {
  ...
  const toggleTool = useMutation({
    mutationFn: ({ server, tool }: { server: McpServer; tool: string }) => {
      ...
      const next = current.includes(tool)
        ...
        : [...current, tool]
      return apiClient.patch<McpServer>(`/api/settings/mcp/${server.id}`, {
...
function RecentEvents({ serverId }: { serverId: string }) {
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ events: McpConnectionEvent[] }>(`/api/settings/mcp/${serverId}/events`),
    retry: false,
  })
...
function McpServerForm({
  ...
  const [httpPreferSse, setHttpPreferSse] = useState(initial?.httpPreferSse ?? false)
  ...
  const save = async () => {
    ...
    try {
      await apiClient.post('/api/settings/mcp', {
        ...
        autoReconnect,
        metadata: initial
          ? { presetId: initial.id, presetBadge: initial.badge, source: 'best-options' }
          : { source: 'custom' },
      })
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-fg">
            {initial ? `Add · ${initial.title}` : 'Add MCP server'}
          </p>
          <p className="text-xs text-fg-muted">
            {initial
              ? 'Best option prefilled — adjust if needed, then create (persists to mcp_servers).'
