...
/**
 * Open Jarvis login page — email/password + OAuth when configured.
 * Microsoft Graph SSO (Office Briefly) is separate from better-auth social providers.
...
function LoginPage() {
  ...
  function startMicrosoftSso() {
    ...
    // Same-tab navigation so Chrome can reuse the work-account SSO session.
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-[11px] leading-4 text-[var(--fgColor-muted)]">
            For Office email / calendar (Microsoft Graph). Uses login.microsoftonline.com in this
            Chrome profile — Open Cowork.exe is not required.
          </p>
          {microsoft && !microsoft.configured ? (
            <p className="text-[11px] leading-4 text-[var(--fgColor-danger)]">
              Missing env: {(microsoft.missingEnv ?? ['MICROSOFT_CLIENT_ID', 'MICROSOFT_CLIENT_SECRET']).join(', ')}
            </p>
          ) : null}
        </div>
